...
extern void himax_ic_reset(uint8_t loadconfig,uint8_t int_off);
	...
	{
		#include "L9560_C01_E501_FF01_20170323_144717.i"
	};
#endif
...
update_retry:
			...
			if(ret == 0){
				...
				sprintf(tp_version, "[fw]0x%2.2X,0x%2.2X [ic]HX8527", ic_data->vendor_fw_ver, ic_data->vendor_config_ver);
...
#ifdef HX_SMART_WAKEUP
	//I("mode = %d;!FAKE_POWER_KEY_SEND = %d;ts->SMWP_enable= %d;!diag_cmd= %d;\n",atomic_read(&ts->suspend_mode),!FAKE_POWER_KEY_SEND,ts->SMWP_enable,!diag_cmd);
...
int himax_chip_common_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	printk("selwyn-----himax_chip_common_probe\n");
...
int himax_chip_common_suspend(struct himax_ts_data *ts)
{
	...
	gpio_set_value(ts->pdata->gpio_3v3_en,0);
...
int himax_chip_common_resume(struct himax_ts_data *ts)
{
	...
	//gpio_direction_output(ts->pdata->gpio_irq,1);
	gpio_set_value(ts->pdata->gpio_3v3_en,1);
...
#endif
#endif
	//gpio_direction_input(ts->pdata->gpio_irq);
